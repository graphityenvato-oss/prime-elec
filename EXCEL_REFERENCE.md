...
Show a preview table + a “dry run” summary (rows parsed, rows with errors). 2. API Route

Create a Next.js API route like route.ts.
It receives the uploaded file (multipart form), parses the Excel, validates columns and values, and returns a summary. 3. Excel Parsing

Use a server-side parser (e.g. xlsx or exceljs) to read the catalog sheet.
Convert rows into normalized records: category_name, brand_name, subcategory_name, page_url, image_url. 4. Validation

Ensure required columns exist.
Validate URLs, required fields, and duplicates.
Optionally normalize (trim, title case, slug). 5. Database Import
...
Use upsert to avoid duplicates based on a unique constraint. 6. Results
